...
class safetyHazard extends BasePage {
    ...
    async enterphonenumber() {
        const countryDropdown = await Locators.getFirstLocator(sms.country(), 10000);
        await Utils.safeClick(countryDropdown);
        await Utils.swipeTillElement(sms.selectIndia(), 15);
        const indiaOption = await Locators.getFirstLocator(sms.selectIndia(), 10000);
        await Utils.safeClick(indiaOption);
        await Utils.enterInputText(sms.enterphonenumber(), SMS.Phonenumber);
    }
    ...
    async clickOnNext() {
        await Utils.swipeTillElement(sms.next(), 3);
