...
class SupplemantryEditInspection extends BasePage {
    ...
    async supplemantryditInspectionSubmission() {
        logger.info("Starting supplementry Inspection submission flow");
    ...
    async clickonSelectedChecksExpand() {
        const expand = await common.waitUntilDisplayed('(//android.widget.TextView[@text=""])[2]')
        await expand.click();
        logger.info('Expanded Checklist');
    }
    async clickCheckklist() {
        const subcheck = await common.waitUntilDisplayed('(//android.widget.TextView[@text="󰄱"])[2]');
    ...
    async editSupplemantryEditInspection() {
        await this.openCreateNewInspection('View Inspections');
        await browser.pause('5000');

        const entryFound = await this.checkInspectionentry('Supplementary inspection');
        ...
        await browser.pause(5000)
        await this.clickonSelectedChecksExpand();
        await this.clickCheckklist();
...
export default new SupplemantryEditInspection();
