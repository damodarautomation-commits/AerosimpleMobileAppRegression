...
class comments_submission extends BasePage {
    async click_on_new() {
        try {
    ...
    async upload_attachment_and_submit_comment(text) {
        const attachment = await $('//android.widget.EditText[@text="Write a comment"]/preceding-sibling::android.view.ViewGroup/android.widget.ImageView');
        await attachment.waitForDisplayed({ timeout: 5000 });
        await attachment.click();

        const gallery = await $('~, GALLERY');
        await gallery.waitForDisplayed({ timeout: 5000 });
        await gallery.click();
        ...
        const sendButton = await $('//android.widget.EditText[@text="Write a comment"]/following-sibling::android.view.ViewGroup/android.widget.ImageView');
        await sendButton.waitForDisplayed({ timeout: 5000 });
        await sendButton.click();

        const validationMsg = await $('//android.widget.TextView[@text="Please enter the comment."]');
        await validationMsg.waitForDisplayed({ timeout: 3000 });
        logger.info('Validation message displayed');

        const commentField = await $('android.widget.EditText');
        await commentField.clearValue();
        await commentField.setValue(text);

        await sendButton.click();
        logger.info(`Comment with attachment submitted: ${text}`);
    }


    async comments_on_form(form_name) {
        try {
            ...
            await this.click_action_item('Comments');
            await browser.pause(20000);
            await this.upload_attachment_and_submit_comment("This is aerosimple automation testing comment from mobile");
            await browser.pause(20000);
            logger.info(`Completed comments_on_form flow for form: ${form_name}`);
        } catch (err) {
            logger.error(`Error in comments_on_form: ${err.stack || err.message}`);
            throw err;
        }
    }
...
export default new comments_submission();
