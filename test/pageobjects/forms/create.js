...
import { creatnew, ClickOnFormName, FormOption, InputTextField, CheckBox } from '../../locators/forms.js';
import { Forms } from '../../testdata/forms.json';
import Utils from '../../utils/common.js'

class New_Form_Submission extends BasePage {

    async click_on_new() {
        try {
            const addBtn = await Locators.getFirstLocator(creatnew(), 10000)
            await addBtn.click();
    ...
    async click_on_form(form_name) {
        try {
            let form = await Locators.getFirstLocator(ClickOnFormName(form_name), 10000)
            if (!(await form.isExisting())) {
                logger.warn(`Form not found by accessibility id, trying XPath...`);
                form = await Locators.getFirstLocator(ClickOnFormName(form_name), 10000)
            }
    ...
    async click_on_formOptions(option) {
        try {
            let opt = await Locators.getFirstLocator(FormOption(option), 10000);
            if (!(await opt.isExisting())) {
                logger.warn(`Option not found by accessibility id, trying XPath...`);
                opt = await Locators.getFirstLocator(FormOption(option), 10000)
            }
    ...
    async name_field(Field, text) {
        try {
            const input = await Locators.getFirstLocator(InputTextField(Field), 10000);
            await Utils.enterInputText(input, text);
    ...
    async number_field(Field, value) {
        try {
            const input = await Locators.getFirstLocator(InputTextField(Field), 10000);
            await Utils.enterInputText(input, value);
    ...
    async click_on_checkbox(Field) {
        try {
            logger.info(`Attempting to click on Checkbox`);
            const checkbox = await Locators.getFirstLocator(CheckBox(Field), 10000)
    ...
    async annotation() {
        try {
            ...
            const annotationImage = await $('(//android.view.ViewGroup//android.widget.ImageView)[last()]');
            await this.swipe_till_element(annotationImage);
            await annotationImage.click();
            logger.info("Opened Annotation screen");

            await browser.pause(5000);

            const clearall = await $('id=com.aerosimple.hybridapp:id/clear_all_tv');
    ...
    async enter_SSI_Field() {
        try {
            ...
            await this.swipe_till_element(ssiField);
    ...
    async form_submission(form_name) {
        try {
            await this.waituntilLoaderToDisappear();
            await browser.pause(3000);
            await this.click_on_new();
            await this.click_on_form(form_name);
            await this.click_on_formOptions(Forms.FormOptions[0]);
            await this.name_field(Forms.FieldName, Forms.FieldNameValue);
            await this.number_field(Forms.FieldNumber, Forms.FieldNumberValue);
            await this.click_on_checkbox(Forms.FieldCheckBox);
            await this.select_date_by_label("Date and Time ", 18, "February", 2025);
            ...
            logger.error(`Error in form_submission: ${err.stack || err.message}`);
            throw err;
        }
    }
}

export default new New_Form_Submission();
